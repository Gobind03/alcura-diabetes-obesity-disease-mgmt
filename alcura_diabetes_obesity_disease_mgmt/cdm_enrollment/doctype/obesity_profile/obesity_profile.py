"""Obesity Profile controller.

One active obesity profile per enrollment. Captures obesity-specific clinical
data and baseline measurements.
"""
...
class ObesityProfile(Document):
	...
	def _validate_no_duplicate_per_enrollment(self) -> None:
		...
		if existing:
			frappe.throw(
				_("Enrollment {0} already has an active Obesity Profile ({1}).").format(
