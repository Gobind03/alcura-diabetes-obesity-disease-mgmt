...
class TestBaselineAssessmentBMI:
	...
	def _make_doc(self, **kwargs):
		from alcura_diabetes_obesity_disease_mgmt.cdm_enrollment.doctype.disease_baseline_assessment.disease_baseline_assessment import (
...
class TestBaselineServiceCreate:
	...
	@patch("alcura_diabetes_obesity_disease_mgmt.services.baseline.BaselineService.identify_care_gaps")
	@patch("alcura_diabetes_obesity_disease_mgmt.services.baseline.BaselineService.prefill_from_healthcare_data")
	@patch("alcura_diabetes_obesity_disease_mgmt.services.baseline.frappe")
...
class TestBaselineServicePrefill:
	...
	def test_prefill_vitals(self, mock_frappe):
		...
		with patch(
			"alcura_diabetes_obesity_disease_mgmt.adapters.vitals_adapter.get_latest_vitals",
			return_value={
				"name": "VS-001",
	...
	def test_prefill_medications(self, mock_frappe):
		...
		with patch(
			"alcura_diabetes_obesity_disease_mgmt.adapters.medication_adapter.get_medication_snapshot",
			return_value=[
...
class TestBaselineCareGaps:
	...
	def test_diabetes_gap_definitions(self):
		from alcura_diabetes_obesity_disease_mgmt.services.baseline import _get_expected_fields_for_disease
