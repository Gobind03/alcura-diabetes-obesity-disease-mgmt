...
class TestGetLatestLabResult:
	@patch("alcura_diabetes_obesity_disease_mgmt.adapters.lab_adapter.frappe")
	@patch("alcura_diabetes_obesity_disease_mgmt.adapters.lab_adapter.require_doctype")
	def test_returns_latest_result(self, mock_require, mock_frappe):
		...
		from alcura_diabetes_obesity_disease_mgmt.adapters.lab_adapter import get_latest_lab_result
...
class TestGetRelevantLabs:
	@patch("alcura_diabetes_obesity_disease_mgmt.adapters.lab_adapter.safe_get_all")
	def test_diabetes_markers(self, mock_get_all):
		mock_get_all.return_value = [{"name": "LT-001", "template": "HbA1c"}]

		from alcura_diabetes_obesity_disease_mgmt.adapters.lab_adapter import get_relevant_labs
...
class TestCheckLabTemplateExists:
	@patch("alcura_diabetes_obesity_disease_mgmt.adapters.lab_adapter.frappe")
	@patch("alcura_diabetes_obesity_disease_mgmt.adapters.lab_adapter.doctype_exists")
	def test_returns_true_when_template_exists(self, mock_dt_exists, mock_frappe):
		mock_dt_exists.return_value = True
		mock_frappe.db.exists.return_value = True

		from alcura_diabetes_obesity_disease_mgmt.adapters.lab_adapter import check_lab_template_exists
