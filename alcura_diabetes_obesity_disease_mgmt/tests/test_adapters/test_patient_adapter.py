...
from alcura_diabetes_obesity_disease_mgmt.adapters.base import CDMDependencyError


class TestGetPatientSummary:
	@patch("alcura_diabetes_obesity_disease_mgmt.adapters.patient_adapter.frappe")
	@patch("alcura_diabetes_obesity_disease_mgmt.adapters.patient_adapter.require_doctype")
	def test_returns_patient_data(self, mock_require, mock_frappe):
		...
		from alcura_diabetes_obesity_disease_mgmt.adapters.patient_adapter import get_patient_summary
...
class TestGetPatientRiskFactors:
	...
	def test_returns_risk_factors(self, mock_require, mock_frappe):
		...
		from alcura_diabetes_obesity_disease_mgmt.adapters.patient_adapter import get_patient_risk_factors
...
class TestSearchPatients:
	@patch("alcura_diabetes_obesity_disease_mgmt.adapters.patient_adapter.safe_get_all")
	def test_search_with_term(self, mock_get_all):
		mock_get_all.return_value = [{"name": "PAT-001", "patient_name": "John Doe"}]

		from alcura_diabetes_obesity_disease_mgmt.adapters.patient_adapter import search_patients
