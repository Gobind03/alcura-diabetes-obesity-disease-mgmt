...
class TestGoalTargetValidation:
	...
	def test_no_target_raises(self):
		from alcura_diabetes_obesity_disease_mgmt.cdm_care_plans.doctype.disease_goal.disease_goal import (
			DiseaseGoal,
		)
...
class TestGoalRevision:
	...
	@patch("alcura_diabetes_obesity_disease_mgmt.cdm_care_plans.doctype.disease_goal.disease_goal.frappe")
...
class TestGoalStatusTransitions:
	...
	def test_not_started_to_in_progress(self):
		from alcura_diabetes_obesity_disease_mgmt.constants.clinical import GOAL_STATUS_TRANSITIONS
