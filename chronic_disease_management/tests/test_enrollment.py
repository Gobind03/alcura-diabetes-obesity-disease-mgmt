...
class TestDiseaseTypeValidation:
	...
	def test_valid_types(self):
		from chronic_disease_management.utils.validators import validate_disease_type
...
class TestEnrollmentServiceEligibility:
	...
	@patch("chronic_disease_management.services.enrollment.frappe")
	def test_eligible_when_no_existing(self, mock_frappe):
		...
		with patch(
			"chronic_disease_management.services.enrollment.EnrollmentService.check_eligibility.__wrapped__",
			side_effect=None,
		):
			pass

		with patch(
			"chronic_disease_management.utils.document_helpers.get_enabled_programs",
...
class TestEnrollmentServiceStatusTransitions:
	...
	def test_close_enrollment_calls_update(self, mock_frappe):
		...
		with patch(
			"chronic_disease_management.utils.validators.validate_enrollment_status_transition"
		):
...
class TestDiseaseEnrollmentController:
	...
	def test_invalid_disease_type_raises(self):
		from chronic_disease_management.cdm_enrollment.doctype.disease_enrollment.disease_enrollment import (
			DiseaseEnrollment,
		)
...
class TestEnrollmentContextBuilder:
	...
	@patch("chronic_disease_management.services.enrollment.frappe")
	@patch("chronic_disease_management.services.enrollment.EnrollmentService.check_existing_enrollment")
	def test_basic_context(self, mock_check, mock_frappe):
		...
		with patch("chronic_disease_management.services.enrollment.date_diff", return_value=16913):
			with patch("chronic_disease_management.services.enrollment.nowdate", return_value="2026-04-22"):
