...
class TestIsCdmClinician:
	@patch("chronic_disease_management.permissions.cdm_permissions.frappe")
	def test_physician_is_clinician(self, mock_frappe):
		...
		from chronic_disease_management.permissions.cdm_permissions import is_cdm_clinician
...
class TestIsCdmAdmin:
	...
	def test_admin_role(self, mock_frappe):
		...
		from chronic_disease_management.permissions.cdm_permissions import is_cdm_admin
...
class TestGetAllowedPatients:
	...
	def test_admin_gets_none_unrestricted(self, mock_frappe):
		...
		from chronic_disease_management.permissions.cdm_permissions import get_allowed_patients
		assert get_allowed_patients() is None

	@patch("chronic_disease_management.permissions.cdm_permissions.get_patient_for_user")
	@patch("chronic_disease_management.permissions.cdm_permissions.is_cdm_admin")
	@patch("chronic_disease_management.permissions.cdm_permissions.is_cdm_clinician")
	@patch("chronic_disease_management.permissions.cdm_permissions.frappe")
...
class TestGetCdmQueryConditions:
	...
	def test_administrator_gets_empty(self, mock_frappe):
		...
		from chronic_disease_management.permissions.cdm_permissions import get_cdm_query_conditions
...
class TestHasCdmPermission:
	...
	def test_administrator_always_allowed(self, mock_frappe):
		...
		from chronic_disease_management.permissions.cdm_permissions import has_cdm_permission
...
class TestValidatePortalAccess:
	...
	def test_allows_own_record(self, mock_frappe, mock_patient_for_user):
		...
		from chronic_disease_management.permissions.cdm_permissions import validate_portal_access
...
class TestRoleMatrix:
	def test_all_doctypes_have_entries(self):
		from chronic_disease_management.permissions.role_matrix import CDM_DOCTYPES, PERMISSION_MATRIX
		assert len(CDM_DOCTYPES) >= 7
		for dt in CDM_DOCTYPES:
			assert dt in PERMISSION_MATRIX

	def test_all_roles_are_valid(self):
		from chronic_disease_management.permissions.role_matrix import validate_permission_matrix
	...
	def test_admin_has_full_crud_on_enrollment(self):
		from chronic_disease_management.permissions.role_matrix import PERMISSION_MATRIX
...
class TestAuditLogging:
	@patch("chronic_disease_management.permissions.audit.frappe")
	def test_log_status_change_creates_comment(self, mock_frappe):
		mock_frappe.session.user = "doc@example.com"
		mock_doc = MagicMock()
		mock_frappe.get_doc.return_value = mock_doc

		from chronic_disease_management.permissions.audit import log_status_change
		log_status_change(
	...
	def test_log_critical_action_creates_comment(self, mock_frappe):
		...
		from chronic_disease_management.permissions.audit import log_critical_action
		log_critical_action(
			"Protocol Override",
	...
	def test_get_audit_trail(self, mock_frappe):
		...
		from chronic_disease_management.permissions.audit import get_audit_trail
